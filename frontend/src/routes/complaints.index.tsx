...
function ComplaintsPage() {
  ...
  useEffect(() => {
    complaintService.list().then(setItems);
  }, []);
    ...
    <AppShell title={t("complaints.title")}>
      <div className="mb-4 flex justify-between">
        <p className="text-muted-foreground">
          Track your public issue reports and verify resolutions.
        </p>
        <Button asChild>
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="active" className="mt-4">
          <List
            items={filter((c) => c.status !== "resolved" && c.status !== "citizen_verification")}
          />
        </TabsContent>
        <TabsContent value="verify" className="mt-4">
          <List items={filter((c) => c.status === "citizen_verification")} />
        </TabsContent>
        <TabsContent value="resolved" className="mt-4">
          <List items={filter((c) => c.status === "resolved")} />
        </TabsContent>
      </Tabs>
    </AppShell>
  );
  ...
  if (items.length === 0)
    return (
      <EmptyState
        ...
        title="No public issues here yet"
        action={
          <Button asChild>
