  ...
  if (m === null)
    return (
      <AppShell>
        <Card className="p-6">
          Mission not found.{" "}
          <Link to="/missions" className="text-primary hover:underline">
            Back
          </Link>
        </Card>
      </AppShell>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <Card className="p-6">
          <div className="flex items-center gap-2">
            <StatusBadge tone={m.status === "completed" ? "success" : "info"}>
              {m.status}
            </StatusBadge>
          ...
          <ol className="mt-6 space-y-3">
            {m.steps.map((step) => {
              ...
              return (
                <li
                  key={step.id}
                  className={`flex gap-4 rounded-xl border p-4 ${complete ? "bg-success/5 border-success/30" : inProgress ? "bg-info/5 border-info/30" : "bg-card"}`}
                >
                  <button
                    type="button"
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-medium ${complete ? "line-through opacity-70" : ""}`}>
                        {step.title}
                      </span>
                    </div>
                    <p className="mt-1 text-sm text-muted-foreground">{step.explanation}</p>
                    {step.relatedDocument && (
                      <div className="mt-2">
                        <StatusBadge tone="warning">Needs: {step.relatedDocument}</StatusBadge>
                      </div>
                    )}
                  </div>
                </li>
          ...
          {m.relatedServiceId && (
            <Card className="p-5">
              ...
              <Button className="mt-3 w-full" asChild variant="outline">
                <Link to="/opportunities/$id" params={{ id: m.relatedServiceId }}>
                  Open service
                </Link>
              </Button>
            </Card>
          ...
          <Card className="p-5">
            ...
            <div className="mt-3 flex flex-col gap-2">
              <Button variant="outline" asChild>
                <Link to="/ai-saathi" search={{ q: `Help me with the mission: ${m.title}` }}>
                  Ask AI Saathi
                </Link>
              </Button>
              <Button variant="outline" asChild>
                <Link to="/docready">Check documents</Link>
              </Button>
            </div>
          </Card>
        </div>
